...
# Geomaps Tensorboard callbacks
@dataclass
class MetricsAdderConf:
    _target_: str = "geomaps.callbacks.tensorboard.MetricsAdder"


@dataclass
class GraphAdderConf:
    _target_: str = "geomaps.callbacks.tensorboard.GraphAdder"


@dataclass
class HistogramAdderConf:
    _target_: str = "geomaps.callbacks.tensorboard.HistogramAdder"


@dataclass
class PredictionsAdderConf:
    _target_: str = "geomaps.callbacks.tensorboard.PredictionsAdder"


@dataclass
class HParamsAdderConf:
    _target_: str = "geomaps.callbacks.tensorboard.HParamsAdder"


# Geomaps Wandb callbacks
@dataclass
class LogConfusionMatrixConf:
    _target_: str = "geomaps.callbacks.wandb.LogConfusionMatrix"


@dataclass
class MetricsWandbConf:
    _target_: str = "geomaps.callbacks.wandb.MetricsWandb"


@dataclass
class UploadCodeAsArtifactConf:
    _target_: str = "geomaps.callbacks.wandb.UploadCodeAsArtifact"
    code_dir: Optional[str] = "wandb_defaults"
    use_git: Optional[bool] = True


# Geomaps utils callbacks
@dataclass
class LightningCheckpointConf:
    _target_: str = "geomaps.callbacks.utils.LightningCheckpoint"
...
class HistorySaverConf:
    _target_: str = "geomaps.callbacks.utils.HistorySaver"


@dataclass
class CustomPredictionWriterConf:
    _target_: str = "geomaps.callbacks.utils.CustomPredictionWriter"
    output_dir: str = MISSING
    output_type: str = MISSING
    write_interval: Optional[str] = "batch"


# Geomaps legacy callbacks (continue learning, handling .pth files, predictions writer)
@dataclass
class ContinueTrainingConf:
    _target_: str = "geomaps.callbacks.legacy.ContinueTraining"
    out_dir: str = MISSING
...
class ExoticCheckPointConf:
    _target_: str = "geomaps.callbacks.legacy.ExoticCheckPoint"
    out_dir: str = MISSING
    out_filename: str = MISSING
    model_out_ext: Optional[str] = ".pth"


# Geomaps callbacks for Tensorboard
@dataclass
class MetricsAdderConf:
    _target_: str = "geomaps.callbacks.tensorboard.MetricsAdder"
