...
from geomaps.data.datasets import PatchDataset
from geomaps.utils.guard import check_files, check_raster_bands
from geomaps.utils.exception import GeomapsError, ErrorCodes
from geomaps.configs.core import DataModuleConf, TransformsConf
...
class SegDataModule(LightningDataModule):
    ...
    def train_dataloader(self):
        if hasattr(self, 'sampling_file') and self.sampling_file is not None:
            import pandas as pd
            import torch
            from torch.utils.data import WeightedRandomSampler
            
            weights = torch.Tensor(
                pd.read_csv(self.sampling_file, header=None)[0].values
            ).type(torch.DoubleTensor)
            
            if len(weights) != len(self.train_image_files):
                LOGGER.error("ERROR: The number of weights in sampling_file does not match the number of training images.")
                raise GeomapsError(ErrorCodes.ERR_CONFIG_HYDRA_ERROR, "Weights length mismatch.")
            
            if self.subset:
                # the subset uses range(0, 20)
                weights = weights[:20]
    ...
    def check_sample(sample):
        if sample['image'].shape[0:-1] != sample['mask'].shape[0:-1]:
            ...
            raise GeomapsError(ErrorCodes.ERR_CONFIG_HYDRA_ERROR,
    ...
    def get_dims_and_meta(self):
        ...
        if self.train_file:
            ...
            raise GeomapsError(ErrorCodes.ERR_CONFIG_HYDRA_ERROR, 'Parameter train_file/test_file is not correct.')
        return dims
    ...
    def get_batch_size(self, parameter_batch_size):
        if isinstance(parameter_batch_size, int):
            ...
            raise GeomapsError(ErrorCodes.ERR_CONFIG_HYDRA_ERROR, 'Parameter batch_size is not correct.')
    ...
    def check_labels(self):
        if self.class_labels is None:
            self.class_labels = [f"class {i}" for i in range(self.num_classes)]
        else:
            if len(self.class_labels) != self.num_classes:
                raise GeomapsError(ErrorCodes.ERR_CONFIG_HYDRA_ERROR,
                                 "ERROR: class_labels parameters should have the same length as the number of classes.")
