...
* classic `U-Net <https://arxiv.org/abs/1505.04597v1>`_ (and Medium/Large variants)
* a light version of the standard Unet with much less parameter
* U-Net with `ResNet`_ as backbone
* `ResBlockNet` a custom U-Net with residual blocks
* Dynamic models from `Segmentation Models Pytorch`_ (SMP)
...
The configuration is composed by Hydra from YAML files in
``geomaps/configs/conf/`` (groups: ``model``, ``datamodule``, ``trainer``,
``callbacks``, ``logger``, ``loss``, ``optimizer``, ``scheduler``,
``transforms``, ``files``, ``experiment``).

Configuration
=============

The Hydra config is split into the groups listed above. A few key fields:
...
* ``sampling_file (string, optional)``:
  path to a CSV file containing weights for each image in the train dataset.
  Used to sample rare classes more frequently via a WeightedRandomSampler.
...
* ``model_name (string)``: 
  name of model to train (values available: 'unet', 'lightunet', 'mediumunet', 
  'largeunet', 'resnet18', 'resnet34', 'resnet50', 'resnet101', 'resnet150',
  'rnet_...', 'smp_...', 'deeplab'). 
...
Variants of U-Net with intermediate (MediumUNet) and large (LargeUNet)
numbers of feature channels to find the best trade-off between speed and accuracy.
...
.. code-block:: json
   ...
   {
    "model": "rnet_16_basicblock_8"
   }

A custom U-Net architecture based on residual blocks (`BasicBlock` or `Bottleneck`)
from torchvision ResNet implementation. The model name dictates the depth and width
(e.g., `rnet_<layer_size>_<block_type>_<n_blocks>`).
...
Dynamic instanciation of models using the `segmentation_models_pytorch` package.
The model name format must be: `smp//<architecture>//<encoder>//<weights>`.
...
.. _Segmentation Models Pytorch: https://github.com/qubvel/segmentation_models.pytorch
